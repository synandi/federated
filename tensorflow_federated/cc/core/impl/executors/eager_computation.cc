...
absl::Status AddInputArgNodesToFunction(
  ...
  for (auto* n : input_args) {
    ...
    if (!status.ok()) {
      return absl::InternalError(absl::StrCat(
          "Failed to crteate input argument node: ", status.ToString()));
    }
...
absl::StatusOr<tensorflow::NodeDef> CreateRelayoutNodeDef(
  ...
  tensorflow::NodeDefBuilder builder(absl::StrCat(varop_node_name, "Relayout"),
                                     "Relayout");
  // tensorflow::OpDefBuilder builder(node->name(), "_Relayout");
...
absl::Status InsertRelayoutNodeForVariable(
    tensorflow::Node* node, tensorflow::dtensor::Layout node_layout,
    tensorflow::Graph* graph) {
  auto relayout_node_def_or = CreateRelayoutNodeDef(node->name(), node_layout);
  ...
  std::vector<tensorflow::Node*> out_nodes;
  for (auto n : node->out_nodes()) {
    out_nodes.push_back(n);
  }
  for (auto n : out_nodes) {
    tf_status = graph->UpdateEdge(relayout_node, 0, n, 0);
    if (!tf_status.ok()) {
      return absl::InternalError(absl::StrCat(
...
absl::Status UpdateVariableLayouts(
    ...
    std::map<std::string, tensorflow::dtensor::Layout> layout_map) {
  // auto library_def =
  // std::make_unique<tensorflow::FunctionLibraryDefinition>(tensorflow::OpRegistry::Global());
  // auto status = library_def->AddFunctionDef(func_def);
  // if (!status.ok()) {
  //   return absl::InternalError(status.ToString());
  // }
  // auto graph =
  //     std::make_unique<tensorflow::Graph>(library_def.get());
